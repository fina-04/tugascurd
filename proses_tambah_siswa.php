...
if($_POST){
    ...
    if(empty($nama_siswa)){
        ...
        include "./koneksi.php";
        $insert=mysqli_query($koneksi,"insert into siswa (nama_siswa,tanggal_lahir, gender, alamat, user_name, password, id_kelas) VALUES ('{$nama_siswa}','{$tanggal_lahir}','{$gender}','{$alamat}','{$user_name}','".md5($password)."','{$id_kelas}')") or die(mysqli_error($koneksi));
        if($insert){
